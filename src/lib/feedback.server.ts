 ...
 */
export async function loadFeedbackList(): Promise<FeedbackListItem[]> {
  const supabase = await createClient();
  const [{ data: rows }, { data: msgs }] = await Promise.all([
    supabase
      .from('sd_feedback')
      .select('id, kind, title, severity, status, page_path, submitted_by, submitted_at, updated_at')
